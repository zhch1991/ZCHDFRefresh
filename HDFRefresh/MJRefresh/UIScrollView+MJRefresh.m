...
/**
 *  调整setionHeader contentInset
 *
 *  @param callback 回调
 *  @param isAdjust 是否调整
 */
-(void)addHeaderWithCallback:(void (^)())callback adjustContentInserForSectionHeader:(BOOL)isAdjust
{
    [self addHeaderWithCallback:callback];
    self.header.isAdjustContentInsetForSectionHeader = isAdjust;
}
...
- (void)addHeaderWithCallbackButWithoutTimeLabel:(void (^)())callback
{
    // 1.创建新的header
    if (!self.header) {
        MJRefreshHeaderView *header = [MJRefreshHeaderView headerWithoutTimeLabel];
        [self addSubview:header];
        self.header = header;
    }
 ...
 * 设置上拉刷新为提示状态，不回调刷新
 */
- (void)setupLoadMoreJust4Notify:(NSString *)notifyInfo
{
    self.footer.userInteractionEnabled = NO;
    self.footer.pullToRefreshText = notifyInfo;
    self.footer.arrowImage.hidden = YES;
}

/***
 * 取消上拉刷新为提示状态，恢复正常的刷新状态
 */
- (void)removeLoadMoreJust4Notify
{
    self.footer.userInteractionEnabled = YES;
    self.footer.pullToRefreshText = MJRefreshFooterPullToRefresh;
    self.footer.arrowImage.hidden = NO;
}
/**
 *  头部
 *
 *  @param notifyInfo
 */
- (void)setupRefreshJust4Notify:(NSString *)notifyInfo
{
    self.header.userInteractionEnabled = NO;
    self.header.pullToRefreshText = notifyInfo;
    self.header.arrowImage.hidden = YES;
}
/**
 *  取消头部
 */
- (void)removeRefreshJust4Notify
{
    self.header.userInteractionEnabled = YES;
    self.header.pullToRefreshText = MJRefreshHeaderPullToRefresh;
    self.header.arrowImage.hidden = NO;
}
/**
 *  文字
 */
